        ...
        if with_respect_to:
            ...
            if ct_model.exists():
                if with_respect_to in args:
                    return sorted(
                        products, key=lambda x: x.__class__._meta.model_name.startswith(
...
class Smartphone(Product):
    ...
    sd_volume_max = models.CharField(
        max_length=255, verbose_name='Максимальный объем встраиваемой памяти', default=0)
...
class CartProduct(models.Model):
    ...
    def __str__(self):
        return 'Продукт: {} (для корзины)'.f(self.product.title)
...
class Customer(models.Model):
    ...
    def __str__(self):
        return 'Пользователь: {} {}'.f(self.user.first_name, self.user.last_name)
